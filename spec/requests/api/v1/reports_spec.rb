...
RSpec.describe '/api/v1/report', type: :request do
  ...
  describe 'POST /' do
    ...
    it 'returns 200 when submitting a nice report' do
      ...
      result = post_json @create_path, { classification: 'nothing', text: text, token: @source.token, reporter: @reporter.uuid }
      expect(response).to have_http_status(201)
      statement = Statement.find(result[:id])
      expect(statement.text).to eq(text)
      expect(statement.classification.slug).to eq('nothing')
    ...
    it 'returns 200 when submitting a hate report' do
      text = 'azd'
      result = post_json @create_path, { classification: 'hate', text: text, token: @source.token, reporter: @reporter.uuid }
      expect(response).to have_http_status(201)
      statement = Statement.find(result[:id])
      expect(statement.text).to eq(text)
      expect(statement.classification.slug).to eq('hate')
      expect(statement.reporter.uuid).to eq(@reporter.uuid)
      expect(statement.reporter.source.token).to eq(@source.token)
    end
    ...
    it 'returns the id of the stored statement' do
      text = 'azd'
      result = post_json '/api/v1/reports', { classification: 'threat', text: text, token: @source.token, reporter: @reporter.uuid }
      expect(response).to have_http_status(201)
      statement = Statement.find(result[:id])
      expect(statement.text).to eq(text)
      expect(statement.classification.slug).to eq('threat')
      expect(statement.reporter.uuid).to eq(@reporter.uuid)
      expect(statement.reporter.source.token).to eq(@source.token)
    end
